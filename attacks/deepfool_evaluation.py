"""
DeepFool Attack Evaluation with FreqPress Defense
DeepFool computes minimal perturbations to cross decision boundaries
"""
import os
import sys
import random
...
BATCH_SIZE = 32  # Smaller batch size for DeepFool
NUM_WORKERS = 0
...
def parse_args():
    """Parse command-line arguments for DeepFool evaluation."""
    parser = argparse.ArgumentParser(
        description="DeepFool evaluation with FreqPress defense"
    )
    ...
    parser.add_argument("--steps", type=int, default=50,
                        help="Maximum DeepFool iterations")
...
def evaluate_with_defense(model, loader, attack, device, cutoff=40.0, order=2, quality=70):
    """
    Evaluate model with FreqPress defense against DeepFool attack
    
    Args:
        model: Neural network model
        loader: Data loader
        attack: DeepFool attack instance
...
def evaluate_without_defense(model, loader, attack, device):
    """
    Evaluate model without defense against DeepFool attack
...
def main():
    ...
    # DeepFool Attack with FreqPress Defense
    print("\n" + "-"*80)
    print("DeepFool Attack - WITH FreqPress Defense")
    print("-"*80)

    print(f"Testing steps={steps}, overshoot={overshoot}...")
    ...
    result_str = f'Model: {model_name}, Steps: {steps}, Overshoot: {overshoot}, Defense: YES, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)

    # DeepFool Attack without Defense
    print("\n" + "-"*80)
    print("DeepFool Attack - WITHOUT Defense")
    print("-"*80)

    print(f"Testing steps={steps}, overshoot={overshoot}...")
    attack = DeepFool(model, steps=steps, overshoot=overshoot)
    acc = evaluate_without_defense(model, loader, attack, device)
    result_str = f'Model: {model_name}, Steps: {steps}, Overshoot: {overshoot}, Defense: NO, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)
    ...
    output_file = os.path.join(output_dir, "deepfool_results.txt")
...
if __name__ == "__main__":
    ...
    # setup_logging("/path/to/deepfool_evaluation.log")
    main()