"""
Carlini-Wagner (C&W) Attack Evaluation with FreqPress Defense
C&W is an optimization-based attack that finds minimal perturbations
"""
import os
import sys
import random
...
BATCH_SIZE = 32  # Smaller batch size for C&W (memory intensive)
NUM_WORKERS = 0
...
def parse_args():
    """Parse command-line arguments for C&W evaluation."""
    parser = argparse.ArgumentParser(
        description="C&W evaluation with FreqPress defense"
    )
...
def evaluate_with_defense(model, loader, attack, device, cutoff=40.0, order=2, quality=70):
    """
    Evaluate model with FreqPress defense against C&W attack
    
    Args:
        model: Neural network model
        loader: Data loader
        attack: C&W attack instance
...
def evaluate_without_defense(model, loader, attack, device):
    """
    Evaluate model without defense against C&W attack
...
def main():
    ...
    # C&W Attack with FreqPress Defense
    print("\n" + "-"*80)
    print("C&W Attack - WITH FreqPress Defense")
    print("-"*80)

    print(f"Testing c={c_val}, steps={steps}...")
    attack = CW(model, c=c_val, kappa=0, steps=steps, lr=0.01)
    acc = evaluate_with_defense(
        model, loader, attack, device,
        cutoff=args.cutoff, order=args.order, quality=args.quality
    )
    result_str = f'Model: {model_name}, c: {c_val}, Steps: {steps}, Defense: YES, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)

    # C&W Attack without Defense
    print("\n" + "-"*80)
    print("C&W Attack - WITHOUT Defense")
    print("-"*80)

    print(f"Testing c={c_val}, steps={steps}...")
    attack = CW(model, c=c_val, kappa=0, steps=steps, lr=0.01)
    acc = evaluate_without_defense(model, loader, attack, device)
    result_str = f'Model: {model_name}, c: {c_val}, Steps: {steps}, Defense: NO, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)
    ...
    output_file = os.path.join(output_dir, "cw_results.txt")
...
if __name__ == "__main__":
    ...
    # setup_logging("/path/to/cw_evaluation.log")
    main()