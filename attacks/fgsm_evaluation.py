"""
FGSM (Fast Gradient Sign Method) Attack Evaluation with FreqPress Defense
"""
import os
import sys
import random
...
from defenses.freqpress import preprocess_batch_butterworth_webp
...
def parse_args():
    """Parse command-line arguments for FGSM evaluation."""
    parser = argparse.ArgumentParser(
        description="FGSM evaluation with FreqPress defense"
    )
...
def evaluate_with_defense(model, loader, attack, device, cutoff=40.0, order=2, quality=70):
    """
    Evaluate model with FreqPress defense against FGSM attack
...
def evaluate_without_defense(model, loader, attack, device):
    """
    Evaluate model without defense against FGSM attack
...
def main():
    ...
    # FGSM Attack with FreqPress Defense
    print("\n" + "-"*80)
    print("FGSM Attack - WITH FreqPress Defense")
    print("-"*80)
    ...
    result_str = f'Model: {model_name}, Epsilon: {eps:.3f}, Defense: YES, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)

    # FGSM Attack without Defense
    print("\n" + "-"*80)
    print("FGSM Attack - WITHOUT Defense")
    print("-"*80)

    attack = FGSM(model, eps=eps)
    acc = evaluate_without_defense(model, loader, attack, device)
    result_str = f'Model: {model_name}, Epsilon: {eps:.3f}, Defense: NO, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)
    ...
    output_file = os.path.join(output_dir, "fgsm_results.txt")
...
if __name__ == "__main__":
    ...
    # setup_logging("/path/to/fgsm_evaluation.log")
    main()