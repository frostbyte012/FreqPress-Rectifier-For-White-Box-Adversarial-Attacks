"""
AutoAttack Evaluation with FreqPress Defense
AutoAttack is an ensemble of diverse attacks for robust evaluation
"""
import os
import sys
import random
...
BATCH_SIZE = 32  # Smaller batch size for AutoAttack (computationally expensive)
NUM_WORKERS = 0
MAX_SAMPLES = None  # Set to None to use full dataset (warning: AutoAttack is slow!)
...
def parse_args():
    """Parse command-line arguments for AutoAttack evaluation."""
    parser = argparse.ArgumentParser(
        description="AutoAttack evaluation with FreqPress defense"
    )
    ...
    parser.add_argument("--epsilon", type=float, default=0.03,
                        help="L-infinity epsilon for AutoAttack")
    parser.add_argument("--version", type=str, default="standard",
                        choices=["standard", "rand"],
                        help="AutoAttack version")
    parser.add_argument("--norm", type=str, default="Linf",
                        choices=["Linf", "L2"],
                        help="Norm used by AutoAttack")
...
def evaluate_with_defense(model, loader, attack, device, cutoff=40.0, order=2, quality=70):
    """
    Evaluate model with FreqPress defense against AutoAttack
    
    Args:
        model: Neural network model
        loader: Data loader
        attack: AutoAttack instance
    ...
    for x, y in loader:
        ...
        print(f"Batch processed: {total} samples, Current accuracy: {correct/total:.4f}")
...
def evaluate_without_defense(model, loader, attack, device):
    """
    Evaluate model without defense against AutoAttack
...
def main():
    ...
    print('Device:', device)
    print("\n⚠️  WARNING: AutoAttack is computationally expensive!")
    print("Consider using MAX_SAMPLES for faster testing.\n")
    ...
    # AutoAttack with FreqPress Defense
    print("\n" + "-"*80)
    print("AutoAttack - WITH FreqPress Defense")
    print("-"*80)

    print(f"\nTesting eps={eps}, version={version}...")
    print("This may take a while...")
    attack = AutoAttack(model, norm=attack_norm, eps=eps, version=version, verbose=False)
    acc = evaluate_with_defense(
        model, loader, attack, device,
        cutoff=args.cutoff, order=args.order, quality=args.quality
    )
    result_str = f'Model: {model_name}, Eps: {eps}, Version: {version}, Defense: YES, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)

    # AutoAttack without Defense
    print("\n" + "-"*80)
    print("AutoAttack - WITHOUT Defense")
    print("-"*80)
    ...
    result_str = f'Model: {model_name}, Eps: {eps}, Version: {version}, Defense: NO, Acc: {acc:.4f}'
    print(result_str)
    results.append(result_str)
    ...
    output_file = os.path.join(output_dir, "autoattack_results.txt")
...
if __name__ == "__main__":
    ...
    # setup_logging("/path/to/autoattack_evaluation.log")
    main()